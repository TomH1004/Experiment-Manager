...
This application provides real-time physiological data acquisition from Polar H10 devices
with integrated remote control capabilities for VR experiment coordination.

Features:
---------
• Real-time heart rate and RR interval monitoring
• Lab Streaming Layer (LSL) integration for live data streaming
• Remote HTTP API for experiment coordination
...
--------------------------
This application includes a built-in Flask HTTP API server that allows remote control
from the VR Experiment Manager system. The API provides endpoints for:

• Health monitoring (GET /api/health)
• Participant ID management (POST /api/participant/set)
• Recording control (POST /api/recording/start, POST /api/recording/stop)
• Interval management (POST /api/interval/start, POST /api/interval/end)
• Timestamp marking (POST /api/timestamp/mark)
...
2. Run the application: python LSL-Lab_Remote.py
3. Connect to Polar H10 device via Bluetooth
4. Set participant ID for data organization
5. Add this LSL-Lab instance to VR Experiment Manager for remote control
...
# Flask imports for remote API server
...
# Add the LSLLabRemoteServer class after the existing imports and before the LSLGui class
class LSLLabRemoteServer:
    """
    Flask API server for remote control of LSL-Lab instances
    Integrates with PolarStreamRecorder for remote recording control
    """
    ...
    def setup_routes(self):
        ...
        def set_participant():
            ...
            try:
                ...
                # Set participant ID using the existing method
                # We need to call this on the main thread
                self.polar_recorder.participant_id_entry.delete(0, tk.END)
                self.polar_recorder.participant_id_entry.insert(0, participant_id)
                
                # Call set_participant_id on main thread
                self.polar_recorder.parent.after(0, self.polar_recorder.set_participant_id)
                
                return jsonify({
                    'success': True,
                    'message': f'Participant ID set to {participant_id}'
                })
                
            except Exception as e:
                return jsonify({
                    'success': False,
                    'message': f'Error setting participant ID: {str(e)}'
                }), 500
        ...
        def start_recording():
            """Start recording"""
            try:
                if not self.polar_recorder.connected:
                    return jsonify({
                        'success': False,
                        'message': 'Device not connected'
                    }), 400
                
                if not self.polar_recorder.current_participant_id:
                    return jsonify({
                        'success': False,
                        'message': 'No participant ID set'
                    }), 400
                ...
                self.polar_recorder.parent.after(0, self.polar_recorder.toggle_recording)
        ...
        def stop_recording():
            """Stop recording"""
            try:
                if not self.polar_recorder.recording:
                    return jsonify({
                        'success': False,
                ...
                self.polar_recorder.parent.after(0, self.polar_recorder.stop_recording)
        ...
        def start_interval():
            ...
            try:
                ...
                if self.polar_recorder.current_interval_start is not None:
                    return jsonify({
                        'success': False,
                        'message': 'Interval already active'
                    }), 400
                
                # Start interval on main thread
                self.polar_recorder.parent.after(0, self.polar_recorder.start_interval)
                ...
                return jsonify({
                    'success': False,
                    'message': f'Error starting interval: {str(e)}'
                }), 500
        ...
        def end_interval():
            ...
            try:
                ...
                if self.polar_recorder.current_interval_start is None:
                    return jsonify({
                        'success': False,
                        'message': 'No active interval'
                    }), 400
                
                # End interval on main thread
                self.polar_recorder.parent.after(0, self.polar_recorder.end_interval)
                ...
                return jsonify({
                    'success': False,
                    'message': f'Error ending interval: {str(e)}'
                }), 500
        ...
        def mark_timestamp():
            ...
            try:
                ...
                # Mark timestamp on main thread
                self.polar_recorder.parent.after(0, self.polar_recorder.mark_timestamp)
                
                message = 'Timestamp marked'
                if timestamp_name:
                ...
                return jsonify({
                    'success': False,
                    'message': f'Error marking timestamp: {str(e)}'
                }), 500
    ...
    def start_server(self):
        ...
        def run_server():
            try:
                print(f"Starting LSL-Lab Remote API Server on {self.host}:{self.port}")
                print(f"API endpoints available:")
                print(f"  - GET  {self.host}:{self.port}/api/health")
                print(f"  - POST {self.host}:{self.port}/api/participant/set")
                print(f"  - POST {self.host}:{self.port}/api/recording/start")
                print(f"  - POST {self.host}:{self.port}/api/recording/stop")
                print(f"  - POST {self.host}:{self.port}/api/interval/start")
                print(f"  - POST {self.host}:{self.port}/api/interval/end")
                print(f"  - POST {self.host}:{self.port}/api/timestamp/mark")
                
                # Update UI status indicator
                if hasattr(self.polar_recorder, 'api_status_label'):
                    self.polar_recorder.parent.after(0, lambda: self.polar_recorder.api_status_label.config(
                        text=f"Remote API: Running on port {self.port}",
                        fg=SUCCESS_COLOR
                    ))
                ...
                print(f"Error starting remote API server: {e}")
                self.running = False
                # Update UI status indicator on error
                if hasattr(self.polar_recorder, 'api_status_label'):
                    self.polar_recorder.parent.after(0, lambda: self.polar_recorder.api_status_label.config(
                        text=f"Remote API: Error - {str(e)[:30]}",
                        fg=ERROR_COLOR
                    ))
        ...
        # Brief delay to let server start, then update status
        time.sleep(0.1)
    ...
    def stop_server(self):
        ...
        if hasattr(self.polar_recorder, 'api_status_label'):
            self.polar_recorder.parent.after(0, lambda: self.polar_recorder.api_status_label.config(
                text="Remote API: Stopped",
                fg=SECONDARY_TEXT
            ))
        
        if self.server_thread and self.server_thread.is_alive():
            # Note: Graceful shutdown of Flask server is complex
            # For now, we rely on daemon threads that will close with the main application
...
class LSLGui:
    def __init__(self, master):
        ...
        # Initialize and start the remote API server
        self.api_server = LSLLabRemoteServer(self.recorder, port=8080)
        self.api_server.start_server()
        
        # Store reference in recorder for cleanup
        self.recorder.api_server = self.api_server
    ...
    def on_closing(self):
        ...
        try:
            ...
            if hasattr(self, 'api_server') and self.api_server:
                print("Stopping remote API server...")
...
class PolarStreamRecorder:
    ...
    def setup_ui(self):
        ...
        # Remote API server status indicator
        self.api_status_label = tk.Label(
            participant_frame,
            text="Remote API: Not started",
            font=("Segoe UI", 9, "italic"),
            bg=DARKER_BG,
            fg=SECONDARY_TEXT,
            anchor="w"
        )
        self.api_status_label.pack(fill=tk.X, pady=(2, 0))
